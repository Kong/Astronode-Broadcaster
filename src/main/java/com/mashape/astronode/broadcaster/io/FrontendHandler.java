...
class FrontendHandler extends ChannelInboundHandlerAdapter implements BackendServerManagerEventListener {
	...
	public void channelActive(ChannelHandlerContext ctx) throws Exception {
		...
		if (bsm.hasBackendServers()) {
			...
			channel.close();
			Log.info(LOG, "No backend servers in pool. Discarding client {}:{}", clientAddress.getHostString(),
	...
	public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
		if (evt instanceof IdleStateEvent) {
			...
			if (event.state() == IdleState.READER_IDLE) {
				Log.info(LOG, "Disconnecting client {}:{} due to idle timeout", clientAddress.getHostString(),
						clientAddress.getPort());
				ChannelUtil.closeOnFlush(ctx.channel());
			}
		}
